...
pub fn export_dae(mesh: &Mesh) -> KernelResult<String> {
    ...
    let _ = writeln!(
        xml,
        "        <source id=\"pos\"><float_array id=\"pos-array\" count=\"{}\">",
        vc * 3
    );
    ...
    let _ = writeln!(
        xml,
        "          <technique_common><accessor source=\"#pos-array\" count=\"{vc}\" stride=\"3\">"
    );
    ...
    let _ = writeln!(
        xml,
        "        <source id=\"norm\"><float_array id=\"norm-array\" count=\"{}\">",
        tc * 3
    );
    ...
    let _ = writeln!(
        xml,
        "          <technique_common><accessor source=\"#norm-array\" count=\"{tc}\" stride=\"3\">"
    );
    xml.push_str("            <param name=\"X\" type=\"float\"/><param name=\"Y\" type=\"float\"/><param name=\"Z\" type=\"float\"/>\n");
    xml.push_str("          </accessor></technique_common></source>\n");
...
pub fn import_dae(content: &str) -> KernelResult<Mesh> {
    ...
    if pos_data.len() % 3 != 0 {
        return Err(KernelError::IoError(
            "DAE: position count not multiple of 3".into(),
        ));
    }
    let vertices: Vec<Point3> = pos_data
        .chunks(3)
    ...
    if indices_flat.len() % 3 != 0 {
        return Err(KernelError::IoError(
            "DAE: index count not multiple of 3".into(),
        ));
    }
    let indices: Vec<[u32; 3]> = indices_flat.chunks(3).map(|c| [c[0], c[1], c[2]]).collect();
