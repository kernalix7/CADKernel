...
#[allow(dead_code)]
pub(crate) fn draw_properties(_ctx: &egui::Context, _gui: &mut GuiState, _scene: &Scene) {
...
fn readonly_row(ui: &mut egui::Ui, label: &str, value: &str, unit: &str) {
    ...
    ui.label(
        egui::RichText::new(value)
            .size(11.0)
            .color(egui::Color32::from_rgb(155, 160, 170)),
    );
    unit_label(ui, unit);
    ui.end_row();
}
...
pub(crate) fn draw_properties_inline(ui: &mut egui::Ui, gui: &mut GuiState, scene: &Scene) {
    // Underline-style Data/View tabs
    {
        let tab_h = 22.0;
        let avail_w = ui.available_width();
        let (tab_rect, _) =
            ui.allocate_exact_size(egui::vec2(avail_w, tab_h), egui::Sense::hover());
        ...
        for (tab_id, label) in &tabs {
            ...
            let area =
                egui::Rect::from_min_size(egui::pos2(x, tab_rect.top()), egui::vec2(tw, tab_h));
            ...
            if is_active {
                painter.line_segment(
                    [
                        ...
                        egui::pos2(x + tw, tab_rect.bottom() - 2.0),
                    ],
...
fn draw_data_tab(
    ...
    if any_label_matches(&shape_labels, filter) {
        collapsible_group(ui, "Shape", true, |ui| {
            let m = &obj.model;
            egui::Grid::new("topo_grid")
                .num_columns(3)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Solids", filter) {
                        readonly_row(ui, "Solids", &format!("{}", m.solids.len()), "");
                    }
                    if matches_filter("Shells", filter) {
                        readonly_row(ui, "Shells", &format!("{}", m.shells.len()), "");
                    }
                    if matches_filter("Faces", filter) {
                        readonly_row(ui, "Faces", &format!("{}", m.faces.len()), "");
                    }
                    if matches_filter("Edges", filter) {
                        readonly_row(ui, "Edges", &format!("{}", m.edges.len()), "");
                    }
                    if matches_filter("Vertices", filter) {
                        readonly_row(ui, "Vertices", &format!("{}", m.vertices.len()), "");
                    }
                    if matches_filter("Triangles", filter) {
                        readonly_row(
                            ui,
                            "Triangles",
                            &format!("{}", obj.mesh.triangle_count()),
                            "",
                        );
                    }
                    if matches_filter("Mesh Verts", filter) {
                        readonly_row(
                            ui,
                            "Mesh Verts",
                            &format!("{}", obj.mesh.vertices.len()),
                            "",
                        );
                    }
                });
    ...
    if any_label_matches(&placement_labels, filter) {
        collapsible_group(ui, "Placement", true, |ui| {
            ...
            egui::Grid::new("placement_pos_grid")
                .num_columns(3)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Position X", filter) {
                        readonly_f64_row(ui, "X", cx, 3, "mm");
                    }
                    if matches_filter("Position Y", filter) {
                        readonly_f64_row(ui, "Y", cy, 3, "mm");
                    }
                    if matches_filter("Position Z", filter) {
                        readonly_f64_row(ui, "Z", cz, 3, "mm");
                    }
                });
            ui.add_space(4.0);
            ui.label(
            ...
            egui::Grid::new("placement_rot_grid")
                .num_columns(3)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Rotation X", filter) {
                        readonly_f64_row(ui, "Angle X", 0.0, 1, "\u{00B0}");
                    }
                    if matches_filter("Rotation Y", filter) {
                        readonly_f64_row(ui, "Angle Y", 0.0, 1, "\u{00B0}");
                    }
                    if matches_filter("Rotation Z", filter) {
                        readonly_f64_row(ui, "Angle Z", 0.0, 1, "\u{00B0}");
                    }
                });
            ...
            if any_label_matches(&bbox_labels, filter) {
                ...
                ui.add_space(4.0);
                ui.label(
                    egui::RichText::new("Bounding Box")
                        .size(10.5)
                        .color(theme::COLOR_DIM)
                        .strong(),
                );
                egui::Grid::new("placement_bbox_grid")
                    .num_columns(3)
                    .spacing([10.0, 4.0])
                    .show(ui, |ui| {
                        if matches_filter("Bounding Width", filter) {
                            readonly_f64_row(ui, "Width", dx, 3, "mm");
                        }
                        if matches_filter("Bounding Depth", filter) {
                            readonly_f64_row(ui, "Depth", dy, 3, "mm");
                        }
                        if matches_filter("Bounding Height", filter) {
                            readonly_f64_row(ui, "Height", dz, 3, "mm");
                        }
                    });
            }
        });
    ...
    if any_label_matches(&computed_labels, filter) {
        collapsible_group(ui, "Computed Properties", true, |ui| {
            ...
            egui::Grid::new("computed_grid")
                ...
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Volume", filter) {
                        readonly_f64_row(ui, "Volume", props.volume, 4, "mm\u{00B3}");
                    }
                    if matches_filter("Surface Area", filter) {
                        readonly_f64_row(ui, "Surface Area", props.surface_area, 4, "mm\u{00B2}");
                    }
                });
            if any_label_matches(
                &["Center of Mass", "Centroid X", "Centroid Y", "Centroid Z"],
                filter,
            ) {
                ui.add_space(2.0);
                ui.label(
                    egui::RichText::new("Center of Mass")
                        .size(10.5)
                        .color(theme::COLOR_DIM)
                        .strong(),
                );
                egui::Grid::new("com_grid")
                    .num_columns(3)
                    .spacing([10.0, 4.0])
                    .show(ui, |ui| {
                        if matches_filter("Centroid X", filter)
                            || matches_filter("Center of Mass", filter)
                        {
                            readonly_f64_row(ui, "X", props.centroid.x, 4, "mm");
                        }
                        if matches_filter("Centroid Y", filter)
                            || matches_filter("Center of Mass", filter)
                        {
                            readonly_f64_row(ui, "Y", props.centroid.y, 4, "mm");
                        }
                        if matches_filter("Centroid Z", filter)
                            || matches_filter("Center of Mass", filter)
                        {
                            readonly_f64_row(ui, "Z", props.centroid.z, 4, "mm");
                        }
                    });
            }
        });
            ...
            egui::Grid::new("display_grid")
                ...
                .show(ui, |ui| {
                    if matches_filter("Display Mode", filter) {
                        ...
                        let current_label = gui
                            .display_mode_override
                            .unwrap_or(DisplayMode::Shading)
                            .label();
                    ...
                    if matches_filter("Transparency", filter) {
                        ...
                        if ui
                            .add(egui::Slider::new(&mut pct, 0..=90).suffix("%").step_by(5.0))
                            .changed()
                        {
                            let mut c = obj.color;
    ...
    if any_label_matches(
        ...
        collapsible_group(ui, "Colors", true, |ui| {
            ...
            if matches_filter("Line Color", filter) {
                ui.add_space(2.0);
                egui::Grid::new("line_color_grid")
                    .num_columns(2)
                    .spacing([10.0, 4.0])
                    .show(ui, |ui| {
                        ui.label(
                            egui::RichText::new("Line Color:")
                                .size(11.0)
                                .color(theme::COLOR_DIM),
                        );
                        ui.label(
                            egui::RichText::new("(same as face)")
                                .size(10.0)
                                .color(theme::COLOR_DIM),
                        );
                        ui.end_row();
            ...
            if matches_filter("Point Color", filter) {
                egui::Grid::new("point_color_grid")
                    .num_columns(2)
                    .spacing([10.0, 4.0])
                    .show(ui, |ui| {
                        ui.label(
                            egui::RichText::new("Point Color:")
                                .size(11.0)
                                .color(theme::COLOR_DIM),
                        );
                        ui.label(
            ...
            egui::Grid::new("visibility_grid")
                ...
                .show(ui, |ui| {
                    if matches_filter("Visibility", filter) {
                        ui.label(
                            egui::RichText::new("Show/Hide:")
                                .size(11.0)
                                .color(theme::COLOR_DIM),
                        );
                        let vis_text = if obj.visible {
                            "\u{25C9} Visible"
                        } else {
                            "\u{25CB} Hidden"
                        };
                        if ui
                            .selectable_label(obj.visible, egui::RichText::new(vis_text).size(11.0))
                            .clicked()
                        {
...
fn draw_color_presets(ui: &mut egui::Ui, gui: &mut GuiState, obj: &crate::scene::SceneObject) {
    ui.horizontal(|ui| {
        ...
        for &(name, color) in presets {
            ...
            if resp.clicked() {
                gui.actions
                    .push(GuiAction::SetObjectColor { id: obj.id, color });
            }
            if resp.hovered() {
...
fn draw_transform_actions(ui: &mut egui::Ui, gui: &mut GuiState, obj: &crate::scene::SceneObject) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui.small_button("Apply").clicked() {
            gui.actions
                .push(GuiAction::ScaleObjectUniform { id, factor });
        }
    });
}
                            ...
                            if let Some(idx) = obj.edge_handles.iter().position(|eh| eh == edge_h) {
                                ...
                                if matches_filter("End", filter) {
                                    readonly_row(
                                        ui,
                                        "End",
                                        &format!("({:.3}, {:.3}, {:.3})", end[0], end[1], end[2]),
                                        "mm",
                                    );
                                }
                    ...
                    egui::Grid::new("sel_vert_grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            if let Some(idx) = obj.vertex_handles.iter().position(|vh| vh == vert_h)
                            {
                                ...
                                if matches_filter("X", filter) {
                                    readonly_f64_row(ui, "X", pos[0] as f64, 6, "mm");
                                }
                                if matches_filter("Y", filter) {
                                    readonly_f64_row(ui, "Y", pos[1] as f64, 6, "mm");
                                }
                                if matches_filter("Z", filter) {
                                    readonly_f64_row(ui, "Z", pos[2] as f64, 6, "mm");
                                }
                            }
                        });
                });
...
fn draw_multi_selection_summary(
    ...
    if any_label_matches(&labels, filter) {
        collapsible_group(
            ui,
            &format!("Selection ({} items)", entities.len()),
            true,
            |ui| {
                egui::Grid::new("multi_sel_grid")
                    .num_columns(3)
                    .spacing([10.0, 4.0])
                    .show(ui, |ui| {
...
fn draw_scene_overview(ui: &mut egui::Ui, scene: &Scene) {
    ...
    if scene.is_empty() {
        ...
        ui.label(
            egui::RichText::new("Create or import a model to start.")
                .size(10.5)
                .color(theme::COLOR_DIM),
        );
    }
    ...
    match params {
        CreationParams::Box {
            ...
            egui::Grid::new("box_params")
                ...
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Width", filter) {
                        param_row(ui, "Width", &mut w, 0.1..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("Height", filter) {
                        param_row(ui, "Height", &mut h, 0.1..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("Depth", filter) {
                        param_row(ui, "Depth", &mut d, 0.1..=1000.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Box {
                        width: w,
                        height: h,
                        depth: d,
                    },
        ...
        CreationParams::Cylinder { radius, height } => {
            ...
            let mut changed = false;
            egui::Grid::new("cyl_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Radius", filter) {
                        param_row(ui, "Radius", &mut r, 0.1..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Height", filter) {
                        param_row(ui, "Height", &mut h, 0.1..=1000.0, 0.1, &mut changed);
                    }
            ...
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Cylinder {
                        radius: r,
                        height: h,
                    },
                });
        ...
        CreationParams::Cone {
            ...
            let mut changed = false;
            egui::Grid::new("cone_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Base Radius", filter) {
                        param_row(ui, "Base Radius", &mut br, 0.01..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Top Radius", filter) {
                        param_row(ui, "Top Radius", &mut tr, 0.0..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Height", filter) {
                        param_row(ui, "Height", &mut h, 0.1..=1000.0, 0.1, &mut changed);
                    }
        ...
        CreationParams::Torus {
            ...
            let mut changed = false;
            egui::Grid::new("tor_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Major Radius", filter) {
                        param_row(ui, "Major Radius", &mut mr, 0.1..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Minor Radius", filter) {
                        param_row(
                            ui,
                            "Minor Radius",
                            &mut mnr,
        ...
        CreationParams::Tube {
            ...
            let mut changed = false;
            egui::Grid::new("tube_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Outer Radius", filter) {
                        param_row(ui, "Outer Radius", &mut or, 0.1..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Inner Radius", filter) {
                        param_row(ui, "Inner Radius", &mut ir, 0.01..=499.0, 0.1, &mut changed);
                    }
                    if matches_filter("Height", filter) {
                        param_row(ui, "Height", &mut h, 0.1..=1000.0, 0.1, &mut changed);
                    }
            ...
            egui::Grid::new("prism_params")
                ...
                .show(ui, |ui| {
                    ...
                    if matches_filter("Sides", filter) {
                        ...
                        changed |= ui
                            .add(egui::DragValue::new(&mut s).range(3..=128))
                            .changed();
                        ui.end_row();
                    }
                });
        ...
        CreationParams::Wedge {
            ...
            let mut changed = false;
            egui::Grid::new("wedge_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("DX", filter) {
                        param_row(ui, "DX", &mut x, 0.1..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("DY", filter) {
                        param_row(ui, "DY", &mut y, 0.1..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("DZ", filter) {
                        param_row(ui, "DZ", &mut z, 0.1..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("DX2", filter) {
                        param_row(ui, "DX2", &mut x2, 0.0..=1000.0, 0.1, &mut changed);
                    }
                    if matches_filter("DY2", filter) {
                        param_row(ui, "DY2", &mut y2, 0.0..=1000.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Wedge {
                        dx: x,
                        dy: y,
                        dz: z,
                        dx2: x2,
        ...
        CreationParams::Ellipsoid { rx, ry, rz } => {
            ...
            let mut changed = false;
            egui::Grid::new("ell_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Radius X", filter) {
                        param_row(ui, "Radius X", &mut x, 0.1..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Radius Y", filter) {
                        param_row(ui, "Radius Y", &mut y, 0.1..=500.0, 0.1, &mut changed);
                    }
                    if matches_filter("Radius Z", filter) {
                        param_row(ui, "Radius Z", &mut z, 0.1..=500.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Ellipsoid {
                        rx: x,
                        ry: y,
                        rz: z,
                    },
        ...
        CreationParams::Helix {
            ...
            egui::Grid::new("helix_params")
                ...
                .show(ui, |ui| {
                    ...
                    if matches_filter("Pitch", filter) {
                        param_row(ui, "Pitch", &mut p, 0.1..=100.0, 0.1, &mut changed);
                    }
                    if matches_filter("Turns", filter) {
                        param_row(ui, "Turns", &mut t, 0.5..=100.0, 0.1, &mut changed);
                    }
                    if matches_filter("Tube Radius", filter) {
                        param_row(ui, "Tube Radius", &mut tr, 0.01..=100.0, 0.05, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Helix {
                        radius: r,
                        pitch: p,
                        turns: t,
                        tube_radius: tr,
        ...
        CreationParams::Imported { path } => {
            ui.label(
                egui::RichText::new(format!("File: {path}"))
                    .size(11.0)
                    .color(theme::COLOR_DIM),
            );
        }
        ...
        CreationParams::Boolean { op } => {
            ui.label(
                egui::RichText::new(format!("Operation: {op}"))
                    .size(11.0)
                    .color(theme::COLOR_DIM),
            );
        }
        CreationParams::Fillet { radius } => {
            let mut r = *radius;
            let mut changed = false;
            egui::Grid::new("fillet_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Radius", filter) {
                        param_row(ui, "Radius", &mut r, 0.01..=100.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Fillet { radius: r },
                });
            }
        }
        CreationParams::Chamfer { distance } => {
            let mut d = *distance;
            let mut changed = false;
            egui::Grid::new("chamfer_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Distance", filter) {
                        param_row(ui, "Distance", &mut d, 0.01..=100.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Chamfer { distance: d },
                });
            }
        }
        CreationParams::Shell { thickness } => {
            let mut t = *thickness;
            let mut changed = false;
            egui::Grid::new("shell_params")
                .num_columns(2)
                .spacing([10.0, 4.0])
                .show(ui, |ui| {
                    if matches_filter("Thickness", filter) {
                        param_row(ui, "Thickness", &mut t, 0.01..=50.0, 0.1, &mut changed);
                    }
                });
            if changed {
                gui.actions.push(GuiAction::RebuildObject {
                    id,
                    params: CreationParams::Shell { thickness: t },
                });
            }
        }
...
fn mesh_center(mesh: &cadkernel_io::Mesh) -> (f64, f64, f64) {
    ...
    let sum = mesh
        .vertices
        .iter()
        .fold((0.0_f64, 0.0_f64, 0.0_f64), |(sx, sy, sz), p| {
...
const MATERIAL_PRESETS: &[MaterialPreset] = &[
    ...
    MaterialPreset {
        name: "Glass",
        color: [0.75, 0.85, 0.90, 0.35],
        icon: "\u{25C7}",
    },
    MaterialPreset {
        name: "Wood (Light)",
        color: [0.72, 0.56, 0.35, 1.0],
        icon: "\u{25A3}",
    },
    MaterialPreset {
        name: "Wood (Dark)",
...
fn draw_material_presets(ui: &mut egui::Ui, gui: &mut GuiState, obj: &crate::scene::SceneObject) {
    // Current material detection (find closest match)
    let current_mat = MATERIAL_PRESETS
        .iter()
        .enumerate()
        .find(|(_, m)| {
