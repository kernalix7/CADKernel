...
fn param_field(
    ...
    let changed = ui
        .add(egui::DragValue::new(value).range(range).speed(speed).suffix(" mm"))
        .changed();
    ui.end_row();
    if !help.is_empty() {
        ui.label("");
        ui.label(egui::RichText::new(help).size(10.0).color(theme::COLOR_DIM).italics());
        ui.end_row();
    }
    changed
}
...
fn button_bar(
    ...
    ui.painter().rect_filled(sep_rect, 0.0, egui::Color32::from_gray(55));
    ui.add_space(6.0);
    ui.horizontal(|ui| {
        // Primary button with accent color
        let btn = ui.add(
            egui::Button::new(egui::RichText::new(ok_label).strong().color(egui::Color32::WHITE))
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            reset = ui
                .add(
                    egui::Button::new(
                        egui::RichText::new("Defaults").size(11.0).color(theme::COLOR_DIM),
                    )
                    .frame(false),
                )
                .clicked();
...
pub(crate) fn draw_create_dialogs(ctx: &egui::Context, gui: &mut GuiState) {
    ...
    if show_box {
        egui::Window::new("Create Box")
            ...
            .show(ctx, |ui| {
                ...
                let mut valid = true;
                egui::Grid::new("box_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Width", &mut gui.create_box_size[0], 0.1..=1000.0, 0.1, "");
                    param_field(ui, "Height", &mut gui.create_box_size[1], 0.1..=1000.0, 0.1, "");
                    param_field(ui, "Depth", &mut gui.create_box_size[2], 0.1..=1000.0, 0.1, "");
                });
                ...
                if ok && valid {
                    ...
                    gui.actions.push(GuiAction::CreateBox { width: w, height: h, depth: d });
                    gui.show_create_box = false;
                }
                if cancel { gui.show_create_box = false; }
                if reset { gui.create_box_size = [10.0, 10.0, 10.0]; }
            });
    }
    ...
    if show_cyl {
        egui::Window::new("Create Cylinder")
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("cyl_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius", &mut gui.create_cylinder_radius, 0.1..=500.0, 0.1, "");
                    param_field(ui, "Height", &mut gui.create_cylinder_height, 0.1..=1000.0, 0.1, "");
                });
                let valid = gui.create_cylinder_radius > 0.0 && gui.create_cylinder_height > 0.0;
                if !valid { validation_error(ui, "Radius and Height must be > 0"); }
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok && valid {
                ...
                if cancel { gui.show_create_cylinder = false; }
                if reset { gui.create_cylinder_radius = 5.0; gui.create_cylinder_height = 10.0; }
            });
    }
    ...
    if show_sph {
        egui::Window::new("Create Sphere")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("sph_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius", &mut gui.create_sphere_radius, 0.1..=500.0, 0.1, "");
                });
                let valid = gui.create_sphere_radius > 0.0;
                if !valid { validation_error(ui, "Radius must be > 0"); }
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok && valid {
                    gui.actions.push(GuiAction::CreateSphere { radius: gui.create_sphere_radius });
                    gui.show_create_sphere = false;
                }
                if cancel { gui.show_create_sphere = false; }
                if reset { gui.create_sphere_radius = 5.0; }
            });
    }
    ...
    if show_cone {
        egui::Window::new("Create Cone")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("cone_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Base Radius", &mut gui.create_cone_base_radius, 0.01..=500.0, 0.1, "");
                    param_field(ui, "Top Radius", &mut gui.create_cone_top_radius, 0.0..=500.0, 0.1, "Set to 0 for pointed cone");
                    param_field(ui, "Height", &mut gui.create_cone_height, 0.1..=1000.0, 0.1, "");
                });
                let valid = gui.create_cone_base_radius > 0.0 && gui.create_cone_height > 0.0;
                if !valid { validation_error(ui, "Base Radius and Height must be > 0"); }
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok && valid {
                ...
                if cancel { gui.show_create_cone = false; }
                if reset { gui.create_cone_base_radius = 5.0; gui.create_cone_top_radius = 0.0; gui.create_cone_height = 10.0; }
            });
    }
    ...
    if show_torus {
        egui::Window::new("Create Torus")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("torus_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Major Radius", &mut gui.create_torus_major_radius, 0.1..=500.0, 0.1, "Distance from center to tube center");
                    param_field(ui, "Minor Radius", &mut gui.create_torus_minor_radius, 0.01..=200.0, 0.05, "Tube cross-section radius");
                });
                let valid = gui.create_torus_major_radius > gui.create_torus_minor_radius;
                if !valid { validation_error(ui, "Major Radius must be > Minor Radius"); }
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok && valid {
                ...
                if cancel { gui.show_create_torus = false; }
                if reset { gui.create_torus_major_radius = 5.0; gui.create_torus_minor_radius = 1.5; }
            });
    }
    ...
    if show_tube {
        egui::Window::new("Create Tube")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("tube_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Outer Radius", &mut gui.create_tube_outer_radius, 0.1..=500.0, 0.1, "");
                    param_field(ui, "Inner Radius", &mut gui.create_tube_inner_radius, 0.01..=499.0, 0.1, "Must be less than Outer Radius");
                    param_field(ui, "Height", &mut gui.create_tube_height, 0.1..=1000.0, 0.1, "");
                });
                let valid = gui.create_tube_outer_radius > gui.create_tube_inner_radius;
                if !valid { validation_error(ui, "Outer Radius must be > Inner Radius"); }
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok && valid {
                ...
                if cancel { gui.show_create_tube = false; }
                if reset { gui.create_tube_outer_radius = 5.0; gui.create_tube_inner_radius = 3.0; gui.create_tube_height = 10.0; }
            });
    }
    ...
    if show_prism {
        egui::Window::new("Create Prism")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("prism_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius", &mut gui.create_prism_radius, 0.1..=500.0, 0.1, "Circumscribed circle radius");
                    param_field(ui, "Height", &mut gui.create_prism_height, 0.1..=1000.0, 0.1, "");
                ...
                if cancel { gui.show_create_prism = false; }
                if reset { gui.create_prism_radius = 5.0; gui.create_prism_height = 10.0; gui.create_prism_sides = 6; }
            });
    }
    ...
    if show_wedge {
        egui::Window::new("Create Wedge")
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("wedge_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Width (DX)", &mut gui.create_wedge_dx, 0.1..=1000.0, 0.1, "");
                    param_field(ui, "Height (DY)", &mut gui.create_wedge_dy, 0.1..=1000.0, 0.1, "");
                    param_field(ui, "Depth (DZ)", &mut gui.create_wedge_dz, 0.1..=1000.0, 0.1, "");
                });
                dialog_section(ui, "Top Dimensions");
                egui::Grid::new("wedge_top_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Top Width", &mut gui.create_wedge_dx2, 0.0..=1000.0, 0.1, "Set to 0 for pyramid");
                    param_field(ui, "Top Depth", &mut gui.create_wedge_dy2, 0.0..=1000.0, 0.1, "");
                });
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok {
                    gui.actions.push(GuiAction::CreateWedge {
                        dx: gui.create_wedge_dx, dy: gui.create_wedge_dy, dz: gui.create_wedge_dz,
                        dx2: gui.create_wedge_dx2, dy2: gui.create_wedge_dy2,
                    });
                    gui.show_create_wedge = false;
                }
                if cancel { gui.show_create_wedge = false; }
                if reset {
                    gui.create_wedge_dx = 10.0; gui.create_wedge_dy = 10.0; gui.create_wedge_dz = 10.0;
                    gui.create_wedge_dx2 = 5.0; gui.create_wedge_dy2 = 5.0;
                }
            });
    }
    ...
    if show_ellipsoid {
        egui::Window::new("Create Ellipsoid")
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new("ellipsoid_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius X", &mut gui.create_ellipsoid_rx, 0.1..=500.0, 0.1, "");
                    param_field(ui, "Radius Y", &mut gui.create_ellipsoid_ry, 0.1..=500.0, 0.1, "");
                    param_field(ui, "Radius Z", &mut gui.create_ellipsoid_rz, 0.1..=500.0, 0.1, "");
                });
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok {
                    gui.actions.push(GuiAction::CreateEllipsoid {
                        rx: gui.create_ellipsoid_rx, ry: gui.create_ellipsoid_ry, rz: gui.create_ellipsoid_rz,
                    });
                    gui.show_create_ellipsoid = false;
                }
                if cancel { gui.show_create_ellipsoid = false; }
                if reset { gui.create_ellipsoid_rx = 5.0; gui.create_ellipsoid_ry = 3.0; gui.create_ellipsoid_rz = 2.0; }
            });
    }
    ...
    if show_helix {
        egui::Window::new("Create Helix")
            ...
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("helix_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius", &mut gui.create_helix_radius, 0.1..=500.0, 0.1, "Helix center radius");
                    param_field(ui, "Pitch", &mut gui.create_helix_pitch, 0.1..=100.0, 0.1, "Distance per turn");
                    param_field_no_unit(ui, "Turns", &mut gui.create_helix_turns, 0.5..=100.0, 0.1);
                    param_field(ui, "Tube Radius", &mut gui.create_helix_tube_radius, 0.01..=100.0, 0.05, "Wire cross-section radius");
                });
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok {
                    gui.actions.push(GuiAction::CreateHelix {
                        radius: gui.create_helix_radius, pitch: gui.create_helix_pitch,
                        turns: gui.create_helix_turns, tube_radius: gui.create_helix_tube_radius,
                    });
                    gui.show_create_helix = false;
                }
                if cancel { gui.show_create_helix = false; }
                if reset {
                    gui.create_helix_radius = 5.0; gui.create_helix_pitch = 3.0;
                    gui.create_helix_turns = 3.0; gui.create_helix_tube_radius = 0.5;
                }
            });
    }
    ...
    fn draw_boolean_dialog(
        ...
        egui::Window::new(title)
            ...
            .show(ctx, |ui| {
                ...
                egui::Grid::new(format!("{title}_grid")).num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Width", &mut gui.bool_box_size[0], 0.1..=500.0, 0.1, "");
                    param_field(ui, "Height", &mut gui.bool_box_size[1], 0.1..=500.0, 0.1, "");
                    param_field(ui, "Depth", &mut gui.bool_box_size[2], 0.1..=500.0, 0.1, "");
                });
                dialog_section(ui, "Offset");
                egui::Grid::new(format!("{title}_off_grid")).num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Offset X", &mut gui.bool_offset[0], -500.0..=500.0, 0.1, "");
                    param_field(ui, "Offset Y", &mut gui.bool_offset[1], -500.0..=500.0, 0.1, "");
                    param_field(ui, "Offset Z", &mut gui.bool_offset[2], -500.0..=500.0, 0.1, "");
                });
                ui.separator();
                ui.horizontal(|ui| {
                    if ui.add(egui::Button::new(egui::RichText::new("Apply").strong())).clicked() {
                        let [w, h, d] = gui.bool_box_size;
                        gui.actions.push(make_action(w, h, d, gui.bool_offset));
                        *show = false;
                    }
                    if ui.button("Cancel").clicked() { *show = false; }
                });
            });
        if !open { *show = false; }
    }
...

    {
        ...
        if show { draw_boolean_dialog(ctx, gui, "Boolean Union", &mut show, make_bool_union); }
        gui.show_boolean_union = show;
    }
    {
        let mut show = gui.show_boolean_subtract;
        if show { draw_boolean_dialog(ctx, gui, "Boolean Subtract", &mut show, make_bool_subtract); }
        gui.show_boolean_subtract = show;
    }
    {
        let mut show = gui.show_boolean_intersect;
        if show { draw_boolean_dialog(ctx, gui, "Boolean Intersect", &mut show, make_bool_intersect); }
    ...
    if show_mirror {
        egui::Window::new("Mirror Solid")
            .collapsible(false).resizable(false).open(&mut show_mirror)
            .show(ctx, |ui| {
                ...
                if ok { gui.actions.push(GuiAction::MirrorSolid(gui.mirror_plane)); gui.show_mirror = false; }
                if cancel { gui.show_mirror = false; }
            });
    }
    ...
    if show_scale {
        egui::Window::new("Scale Solid")
            .collapsible(false).resizable(false).open(&mut show_scale)
            .show(ctx, |ui| {
                dialog_section(ui, "Scale Factor");
                egui::Grid::new("scale_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field_no_unit(ui, "Factor", &mut gui.scale_factor, 0.01..=100.0, 0.01);
                });
                if gui.scale_factor <= 0.0 { validation_error(ui, "Factor must be > 0"); }
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok && gui.scale_factor > 0.0 {
                    gui.actions.push(GuiAction::ScaleSolid { factor: gui.scale_factor });
                    gui.show_scale = false;
                }
                if cancel { gui.show_scale = false; }
                if reset { gui.scale_factor = 2.0; }
            });
    }
    ...
    if show_shell {
        egui::Window::new("Shell Solid")
            .collapsible(false).resizable(false).open(&mut show_shell)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("shell_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Thickness", &mut gui.shell_thickness, 0.01..=50.0, 0.1, "Wall thickness");
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                    gui.actions.push(GuiAction::ShellSolid { thickness: gui.shell_thickness });
                    gui.show_shell = false;
                }
                if cancel { gui.show_shell = false; }
                if reset { gui.shell_thickness = 1.0; }
            });
    }
    ...
    if show_fillet {
        egui::Window::new("Fillet All Edges")
            .collapsible(false).resizable(false).open(&mut show_fillet)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("fillet_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Radius", &mut gui.fillet_radius, 0.01..=50.0, 0.1, "Fillet radius");
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                    gui.actions.push(GuiAction::FilletAllEdges { radius: gui.fillet_radius });
                    gui.show_fillet = false;
                }
                if cancel { gui.show_fillet = false; }
                if reset { gui.fillet_radius = 1.0; }
            });
    }
    ...
    if show_chamfer {
        egui::Window::new("Chamfer All Edges")
            .collapsible(false).resizable(false).open(&mut show_chamfer)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("chamfer_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Distance", &mut gui.chamfer_distance, 0.01..=50.0, 0.1, "Chamfer distance");
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                    gui.actions.push(GuiAction::ChamferAllEdges { distance: gui.chamfer_distance });
                    gui.show_chamfer = false;
                }
                if cancel { gui.show_chamfer = false; }
                if reset { gui.chamfer_distance = 1.0; }
            });
    }
    ...
    if show_pattern {
        egui::Window::new("Linear Pattern")
            .collapsible(false).resizable(false).open(&mut show_pattern)
            .show(ctx, |ui| {
                dialog_section(ui, "Pattern Parameters");
                egui::Grid::new("pattern_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    ui.label(egui::RichText::new("Count:").size(11.0).color(theme::COLOR_DIM));
                    ...
                    ui.end_row();
                    param_field(ui, "Spacing", &mut gui.pattern_spacing, 0.1..=200.0, 0.1, "Distance between copies");
                    ui.label(egui::RichText::new("Axis:").size(11.0).color(theme::COLOR_DIM));
                    ui.horizontal(|ui| {
                ...
                if ok {
                    gui.actions.push(GuiAction::LinearPattern {
                        count: gui.pattern_count, spacing: gui.pattern_spacing, axis: gui.pattern_axis,
                    });
                    gui.show_pattern = false;
                }
                if cancel { gui.show_pattern = false; }
                if reset { gui.pattern_count = 3; gui.pattern_spacing = 15.0; gui.pattern_axis = 0; }
            });
    }
    ...
    if show_smooth {
        egui::Window::new("Smooth Mesh")
            .collapsible(false).resizable(false).open(&mut show_smooth)
            .show(ctx, |ui| {
                dialog_section(ui, "Smoothing Parameters");
                egui::Grid::new("smooth_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    ui.label(egui::RichText::new("Iterations:").size(11.0).color(theme::COLOR_DIM));
                    ...
                    ui.end_row();
                    param_field_no_unit(ui, "Factor", &mut gui.mesh_smooth_factor, 0.01..=1.0, 0.01);
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                ...
                if reset { gui.mesh_smooth_iters = 3; gui.mesh_smooth_factor = 0.5; }
            });
    }
    ...
    if show_remesh {
        egui::Window::new("Remesh")
            .collapsible(false).resizable(false).open(&mut show_remesh)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("remesh_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Target Edge Length", &mut gui.mesh_remesh_edge_len, 0.1..=100.0, 0.1, "Smaller = more detail");
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                ...
                if reset { gui.mesh_remesh_edge_len = 1.0; }
            });
    }
    ...
    if show_pad {
        egui::Window::new("Pad (Additive Extrude)")
            .collapsible(false).resizable(false).open(&mut show_pad)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("pad_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Depth", &mut gui.pad_depth, 0.01..=500.0, 0.1, "Extrusion depth");
                });
                ...
                if reset { gui.pad_depth = 10.0; gui.pad_symmetric = false; }
            });
    }
    ...
    if show_pocket {
        egui::Window::new("Pocket (Subtractive Extrude)")
            .collapsible(false).resizable(false).open(&mut show_pocket)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("pocket_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Depth", &mut gui.pocket_depth, 0.01..=500.0, 0.1, "Cut depth");
                });
                ...
                if reset { gui.pocket_depth = 5.0; gui.pocket_through_all = false; }
            });
    }
    ...
    if show_groove {
        egui::Window::new("Groove (Subtractive Revolve)")
            .collapsible(false).resizable(false).open(&mut show_groove)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("groove_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    ui.label(egui::RichText::new("Angle:").size(11.0).color(theme::COLOR_DIM));
                    ui.add(egui::DragValue::new(&mut gui.groove_angle).speed(1.0).range(1.0..=360.0).suffix("°"));
                    ui.end_row();
                });
                ...
                if reset { gui.groove_angle = 360.0; }
            });
    }
    ...
    if show_hole {
        egui::Window::new("Hole")
            .collapsible(false).resizable(false).open(&mut show_hole)
            .show(ctx, |ui| {
                dialog_section(ui, "Hole Parameters");
                egui::Grid::new("hole_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                ...
                if gui.hole_countersink {
                    egui::Grid::new("csink_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                        ui.label(egui::RichText::new("Countersink Angle:").size(11.0).color(theme::COLOR_DIM));
                        ui.add(egui::DragValue::new(&mut gui.hole_countersink_angle).speed(1.0).range(30.0..=120.0).suffix("°"));
                        ui.end_row();
                    });
                }
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                ...
                if cancel { gui.show_hole = false; }
                if reset { gui.hole_radius = 2.0; gui.hole_depth = 10.0; gui.hole_countersink = false; gui.hole_countersink_angle = 90.0; }
            });
    }
    ...
    if show_sprocket {
        egui::Window::new("Create Sprocket")
            .collapsible(false).resizable(false).open(&mut show_sprocket)
            .show(ctx, |ui| {
                dialog_section(ui, "Sprocket Parameters");
                egui::Grid::new("sprocket_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    ui.label(egui::RichText::new("Teeth:").size(11.0).color(theme::COLOR_DIM));
                    ...
                    ui.end_row();
                    param_field(ui, "Roller Diameter", &mut gui.sprocket_roller_diameter, 0.1..=100.0, 0.1, "");
                    param_field(ui, "Pitch", &mut gui.sprocket_pitch, 0.1..=200.0, 0.1, "Chain pitch distance");
                    param_field(ui, "Bore Radius", &mut gui.sprocket_bore, 0.1..=100.0, 0.1, "Center bore radius");
                });
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok {
                ...
                if reset { gui.sprocket_teeth = 18; gui.sprocket_roller_diameter = 10.16; gui.sprocket_pitch = 15.875; gui.sprocket_bore = 5.0; }
            });
    }
    ...
    if show_shaft {
        egui::Window::new("Shaft Design")
            .collapsible(false).resizable(false).open(&mut show_shaft)
            .show(ctx, |ui| {
                ...
                for (i, seg) in gui.shaft_segments.iter_mut().enumerate() {
                    ui.horizontal(|ui| {
                        ...
                        ui.add(egui::DragValue::new(&mut seg.0).speed(0.5).prefix("L=").range(0.1..=500.0).suffix(" mm"));
                        ui.add(egui::DragValue::new(&mut seg.1).speed(0.5).prefix("D=").range(0.1..=500.0).suffix(" mm"));
                        if ui.small_button("\u{2716}").on_hover_text("Remove segment").clicked() {
                            remove_idx = Some(i);
                        }
                    });
                }
                if let Some(idx) = remove_idx {
                    if gui.shaft_segments.len() > 1 { gui.shaft_segments.remove(idx); }
                }
                ...
                if reset { gui.shaft_segments = vec![(20.0, 10.0), (30.0, 15.0), (20.0, 10.0)]; }
            });
    }
    ...
    if show_gear {
        egui::Window::new("Involute Gear")
            .collapsible(false).resizable(false).open(&mut show_gear)
            .show(ctx, |ui| {
                dialog_section(ui, "Gear Parameters");
                egui::Grid::new("gear_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    ui.label(egui::RichText::new("Teeth:").size(11.0).color(theme::COLOR_DIM));
                    let mut t_i32 = gui.gear_teeth as i32;
                    ui.add(egui::DragValue::new(&mut t_i32).range(6..=200));
                    gui.gear_teeth = t_i32.max(6) as u32;
                    ui.end_row();
                    param_field(ui, "Module", &mut gui.gear_module, 0.1..=50.0, 0.1, "Tooth size parameter");
                    ui.label(egui::RichText::new("Pressure Angle:").size(11.0).color(theme::COLOR_DIM));
                    ui.add(egui::DragValue::new(&mut gui.gear_pressure_angle).speed(0.5).range(10.0..=30.0).suffix("°"));
                    ui.end_row();
                });
                let (ok, cancel, reset) = button_bar(ui, "Create");
                if ok {
                ...
                if reset { gui.gear_teeth = 20; gui.gear_module = 2.0; gui.gear_pressure_angle = 20.0; }
            });
    }
    ...
    if show_def {
        egui::Window::new("Auto Defeaturing")
            .collapsible(false).resizable(false).open(&mut show_def)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("defeature_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field(ui, "Size Threshold", &mut gui.defeaturing_threshold, 0.01..=100.0, 0.1, "Removes features smaller than threshold");
                });
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok {
                ...
                if reset { gui.defeaturing_threshold = 1.0; }
            });
    }
        ...
        egui::Window::new("Export Options")
            ...
            .show(ctx, |ui| {
                if let Some(path) = &gui.export_path {
                    ...
                    egui::Grid::new("export_opts_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                        if ext == "stl" {
                            ui.label(egui::RichText::new("Format:").size(11.0).color(theme::COLOR_DIM));
                            ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(format!(
                            "Format: {} | Scale: {:.2}x",
                            if ext == "stl" { if gui.export_stl_binary { "Binary STL" } else { "ASCII STL" } } else { &ext },
                            gui.export_scale
                        ))
                        .size(10.0)
    ...
    if show_fem_mat {
        egui::Window::new("FEM Material")
            .collapsible(false).resizable(false).open(&mut show_fem_mat)
            .show(ctx, |ui| {
                ...
                egui::ComboBox::from_id_salt("fem_mat_preset")
                    ...
                    .show_ui(ui, |ui| {
                        for mat in ["Steel", "Aluminum", "Titanium", "Copper", "Brass", "Cast Iron", "Custom"] {
    ...
    if show_fem_mesh {
        egui::Window::new("FEM Mesh Generation")
            .collapsible(false).resizable(false).open(&mut show_fem_mesh)
            .show(ctx, |ui| {
                dialog_section(ui, "Mesh Parameters");
                egui::Grid::new("fem_mesh_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
    ...
    if show_fem_bc {
        egui::Window::new("FEM Constraint")
            .collapsible(false).resizable(false).open(&mut show_fem_bc)
            .show(ctx, |ui| {
                ...
                egui::ComboBox::from_id_salt("fem_bc_type")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Fixed, "Fixed");
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Force, "Force");
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Pressure, "Pressure");
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Displacement, "Displacement");
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Gravity, "Gravity");
                        ui.selectable_value(&mut gui.fem_constraint_type, FemConstraintType::Spring, "Spring");
                    });
                if !matches!(gui.fem_constraint_type, super::FemConstraintType::Fixed) {
                    egui::Grid::new("fem_bc_val_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                        param_field_no_unit(ui, "Value", &mut gui.fem_constraint_value, -1e6..=1e6, 1.0);
                    });
                }
                let (ok, cancel, _) = button_bar(ui, "Apply");
    ...
    if show_fem_sol {
        egui::Window::new("FEM Solver Settings")
            .collapsible(false).resizable(false).open(&mut show_fem_sol)
            .show(ctx, |ui| {
                dialog_section(ui, "Solver Parameters");
                egui::Grid::new("fem_solver_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                    param_field_no_unit(ui, "Tolerance", &mut gui.fem_solver_tolerance, 1e-12..=1e-2, 1e-7);
                    ui.label(egui::RichText::new("Max Iterations:").size(11.0).color(theme::COLOR_DIM));
                ...
                let (ok, cancel, reset) = button_bar(ui, "Apply");
                if ok { gui.show_fem_solver = false; }
                if cancel { gui.show_fem_solver = false; }
                if reset { gui.fem_solver_tolerance = 1e-6; gui.fem_solver_max_iter = 1000; }
            });
    }
    ...
    if show_explode {
        egui::Window::new("Exploded View")
            .collapsible(false).resizable(false).open(&mut show_explode)
            .show(ctx, |ui| {
                dialog_section(ui, "Parameters");
                egui::Grid::new("explode_grid").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                ...
                if reset { gui.explode_factor = 2.0; }
            });
    }
...
pub(crate) fn draw_settings(ctx: &egui::Context, gui: &mut GuiState, nav: &mut NavConfig) {
    ...
    egui::Window::new("Preferences")
        ...
        .open(&mut show)
        .show(ctx, |ui| {
            let active_tab: SettingsTab = ui.data_mut(|d| *d.get_temp_mut_or(tab_id, SettingsTab::General));
                ...
                ui.vertical(|ui| {
                    ...
                    for &tab in SettingsTab::ALL {
                        ...
                        let label = egui::RichText::new(format!("{} {}", tab.icon(), tab.label()))
                            .size(12.0);
                        let label = if selected { label.strong().color(theme::COLOR_ACCENT) } else { label };
                    ...
                    if ui.add(
                        egui::Button::new(
                            egui::RichText::new("Reset All").size(11.0).color(theme::COLOR_DIM),
                        ).frame(false),
                    ).clicked() {
                        *nav = NavConfig::new();
...
fn settings_heading(ui: &mut egui::Ui, label: &str) {
    ui.add_space(2.0);
    ui.label(egui::RichText::new(label).size(14.0).strong().color(theme::COLOR_ACCENT));
    ui.separator();
}
...
fn draw_settings_general(ui: &mut egui::Ui, nav: &mut NavConfig) {
    ...
    egui::Grid::new("units_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
    ...
    ui.indent("files_indent", |ui| {
        ...
        if nav.auto_save_enabled {
            egui::Grid::new("autosave_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                ui.label("Interval:");
                ui.add(
        ...
        ui.add_space(4.0);
        egui::Grid::new("files_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
...
fn draw_settings_display(ui: &mut egui::Ui, nav: &mut NavConfig) {
    ...
    ui.indent("bg_indent", |ui| {
        egui::Grid::new("bg_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
            ui.label("Preset:");
            ...
            if nav.bg_preset == BgPreset::Custom {
                ...
                if ui.color_edit_button_srgb(&mut top).changed() {
                    nav.bg_custom_top = [top[0] as f32 / 255.0, top[1] as f32 / 255.0, top[2] as f32 / 255.0];
                }
                ui.end_row();
                ...
                if ui.color_edit_button_srgb(&mut bot).changed() {
                    nav.bg_custom_bottom = [bot[0] as f32 / 255.0, bot[1] as f32 / 255.0, bot[2] as f32 / 255.0];
                }
                ui.end_row();
            }
        });
    ...
    ui.indent("overlays_indent", |ui| {
        ...
        if nav.show_grid_3d {
            egui::Grid::new("grid_settings").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
    ...
    ui.indent("cam_indent", |ui| {
        egui::Grid::new("cam_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
            ui.label("Default projection:");
            ui.horizontal(|ui| {
                ui.radio_value(&mut nav.default_projection, Projection::Perspective, "Perspective");
                ui.radio_value(&mut nav.default_projection, Projection::Orthographic, "Orthographic");
            });
            ui.end_row();
        });
    });
    ...
    ui.indent("sel_indent", |ui| {
        egui::Grid::new("sel_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
            ui.label("Selection color:");
            let mut sel_c = egui::Color32::from_rgb(
                nav.selection_color[0], nav.selection_color[1], nav.selection_color[2],
            );
            ...
            let mut pre_c = egui::Color32::from_rgb(
                nav.preselection_color[0], nav.preselection_color[1], nav.preselection_color[2],
            );
    ...
    ui.indent("tess_indent", |ui| {
        egui::Grid::new("tess_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
            ui.label("Default segments:");
            ui.add(
                egui::Slider::new(&mut nav.tessellation_segments, 8..=256)
                    .logarithmic(true),
            );
            ui.end_row();
        });
    ...
    ui.indent("section_indent", |ui| {
        ...
        if nav.clip_enabled {
            egui::Grid::new("section_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
...
fn draw_settings_navigation(ui: &mut egui::Ui, nav: &mut NavConfig) {
    ...
    ui.indent("orbit_rot_indent", |ui| {
        egui::Grid::new("orbit_rot_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
        ...
        ui.checkbox(&mut nav.show_rotation_center, "Show rotation center indicator");
    ...
    ui.indent("sens_indent", |ui| {
        egui::Grid::new("sens_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
            ui.label("Orbit:");
            ui.add(egui::Slider::new(&mut nav.orbit_sensitivity, 0.001..=0.02).logarithmic(true));
            ui.end_row();
            ui.label("Pan:");
            ui.add(egui::Slider::new(&mut nav.pan_sensitivity, 0.0005..=0.01).logarithmic(true));
            ui.end_row();
            ui.label("Zoom:");
        ...
        ui.checkbox(&mut nav.disable_touch_tilt, "Disable touchscreen tilt gesture");
    });
    ...
    ui.indent("anim_indent", |ui| {
        ...
        if nav.enable_view_animation {
            egui::Grid::new("anim_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                ui.label("Duration:");
                ui.add(
    ...
    ui.indent("cube_indent", |ui| {
        ...
        if nav.show_view_cube {
            egui::Grid::new("cube_settings").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                ui.label("Orbit steps:");
                ui.add(egui::DragValue::new(&mut nav.orbit_steps).range(2..=24).suffix(" steps"));
                ui.end_row();
                ui.label("Cube size:");
                ui.add(egui::DragValue::new(&mut nav.cube_size).range(60..=200).suffix(" px"));
                ui.end_row();
                ...
                egui::ComboBox::from_id_salt("cube_corner")
                    .selected_text(match nav.cube_corner {
                        1 => "Top-Left", 2 => "Bottom-Left", 3 => "Bottom-Right", _ => "Top-Right",
                    })
                    .show_ui(ui, |ui| {
...
fn draw_settings_appearance(ui: &mut egui::Ui, gui: &mut GuiState, nav: &mut NavConfig) {
    ...
    ui.indent("theme_indent", |ui| {
        ui.horizontal(|ui| {
            if ui.radio_value(&mut nav.theme_mode, super::theme::ThemeMode::Dark, "Dark").changed()
                || ui.radio_value(&mut nav.theme_mode, super::theme::ThemeMode::Light, "Light").changed()
            {
                gui.theme_applied = false;
            }
        });
    ...
    ui.indent("density_indent", |ui| {
        ui.horizontal(|ui| {
            if ui.radio_value(&mut nav.ui_density, super::theme::UiDensity::Compact, "Compact").changed()
                || ui.radio_value(&mut nav.ui_density, super::theme::UiDensity::Normal, "Normal").changed()
                || ui.radio_value(&mut nav.ui_density, super::theme::UiDensity::Spacious, "Spacious").changed()
            {
                gui.theme_applied = false;
            }
        });
...
fn draw_settings_lighting(ui: &mut egui::Ui, nav: &mut NavConfig) {
    ...
    ui.indent("light_indent", |ui| {
        ...
        if nav.enable_lighting {
            ui.add_space(4.0);
            egui::Grid::new("light_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                ui.label("Intensity:");
            ...
            egui::Grid::new("light_dir_grid").num_columns(2).spacing([12.0, 6.0]).show(ui, |ui| {
                ui.label("X:");
                ui.add(egui::DragValue::new(&mut nav.light_dir[0]).speed(0.01).range(-1.0..=1.0));
                ui.end_row();
                ui.label("Y:");
                ui.add(egui::DragValue::new(&mut nav.light_dir[1]).speed(0.01).range(-1.0..=1.0));
                ui.end_row();
                ui.label("Z:");
                ui.add(egui::DragValue::new(&mut nav.light_dir[2]).speed(0.01).range(-1.0..=1.0));
                ui.end_row();
            });
            ui.add_space(4.0);
...
fn draw_all_shortcuts(ui: &mut egui::Ui) {
    shortcut_section(ui, "\u{1F4C1}", "File", &[
        ("Ctrl+N", "New project"),
        ("Ctrl+O", "Open file"),
        ("Ctrl+S", "Save file"),
    ]);
    shortcut_section(ui, "\u{270F}", "Edit", &[
        ("Ctrl+Z", "Undo"),
    ...
    shortcut_section(ui, "\u{1F3AF}", "Navigation", &[
    ...
    shortcut_section(ui, "\u{1F441}", "Standard Views", &[
        ("1 / Ctrl+1", "Front / Back"),
        ("3 / Ctrl+3", "Right / Left"),
        ("7 / Ctrl+7", "Top / Bottom"),
        ("0", "Isometric"),
    ...
    shortcut_section(ui, "\u{1F5BC}", "Display Modes (V, then...)", &[
        ("V, 1", "As Is"),
        ("V, 2", "Points"),
        ("V, 3", "Wireframe"),
        ("V, 4", "Hidden Line"),
    ...
    shortcut_section(ui, "\u{21C4}", "Transform Gizmo", &[
        ("W", "Move gizmo"),
        ("E", "Rotate gizmo"),
        ("R", "Scale gizmo"),
        ("Drag axis handle", "Transform along axis"),
    ]);
    shortcut_section(ui, "\u{1F50D}", "Selection Modes", &[
        ("2", "Face selection"),
        ("4", "Vertex selection"),
        ("C", "Clear measurement"),
    ]);
    shortcut_section(ui, "\u{2712}", "Sketcher (in sketch mode)", &[
        ("S", "Select tool"),
        ("L", "Line tool"),
        ("R", "Rectangle tool"),
        ("C", "Circle tool"),
...
fn draw_settings_shortcuts(ui: &mut egui::Ui) {
    ...
    ui.weak("All current keyboard shortcuts. Custom key binding will be available in a future release.");
    ui.add_space(4.0);
    draw_all_shortcuts(ui);
}
                ...
                egui::Grid::new("plugin_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        for (_, name, version, state) in &plugins {
                            ...
                            ui.label(
                                egui::RichText::new(state).size(11.0).color(state_color),
                            );
    ...
    egui::Grid::new(format!("shortcuts_{title}"))
        ...
        .show(ui, |ui| {
            for (key, desc) in shortcuts {
                ...
                let key_text = egui::RichText::new(*key).size(11.0).monospace().strong()
                    .color(egui::Color32::from_rgb(200, 210, 225));
                ui.label(key_text);
                ui.label(egui::RichText::new(*desc).size(11.0).color(egui::Color32::from_rgb(160, 168, 180)));
                ui.end_row();
            }
        });
}
