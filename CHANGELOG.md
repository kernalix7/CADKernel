...
- **`cadk::autosave::list_snapshots(dir) -> ApiResult<Vec<AutosaveEntry>>`** — enumerates `*.cadk` files in `dir`, calls `cadk::inspect_path` on each to extract the canonical hash from the document's command count (used as a proxy fingerprint), and returns entries sorted newest-first. **`cadk::autosave::prune(dir, max_snapshots)`** — deletes the oldest entries beyond `max_snapshots`, leaving the directory at most `max_snapshots` files. **`cadk::autosave::recover_latest(dir) -> ApiResult<Option<Vec<u8>>>`** — returns the raw `.cadk` bytes of the newest entry for the caller to pass directly into `Session::load_cadk`.
- **`Session::write_autosave_snapshot(&policy) -> ApiResult<PathBuf>`** — serialises the current session state via `save_cadk()`, writes to `<policy.dir>/<canonical_hash>_<timestamp>.cadk`, then calls `prune` to enforce `max_snapshots`. Returns the path written.
- **Viewer integration** — `AppState` gains an `AutosavePolicy` field (default: 60-second interval, 5 snapshots). The main event loop increments a tick counter and calls `session.write_autosave_snapshot(&policy)` whenever the counter reaches `interval_secs`. On startup the viewer calls `cadk::autosave::recover_latest` and, when a snapshot is found whose hash differs from an empty session, presents a **recovery modal dialog** ("Unsaved work detected — recover?" / "Discard") built with `egui::Window`. Choosing Recover replays the snapshot via `Session::load_cadk`; Discard leaves the session empty.
- **Tests**: 29 new unit + integration tests across `cadkernel-api` (canonical_hash determinism, policy defaults, list/prune/recover lifecycle, snapshot naming convention) and `cadkernel-viewer` (modal visibility conditions, tick-based trigger). Workspace total: **3,254 passed / 0 failed / 1 ignored** (was 3,225).
- STOP_LIST clean (no new `Command` / `Outcome` variants, no new format crate).

#### Commercial CAD Roadmap — A3.0.7: `cadk::migrate` module + `SchemaVersion` enum (2026-05-13)
- **`crates/api/src/cadk/migrate.rs`** — new module splits the schema-version dispatch out of inline checks. Defines `SchemaVersion::V1` (currently the only variant; carries integer `1` on disk) with `from_u32` / `as_u32` / `current()` accessors, plus `cadk::migrate_to_current(bytes) -> ApiResult<Vec<u8>>` — the migrator entry point. As of this slice the format has exactly one supported version so migration is always a no-op for healthy bytes; the module exists to pin the contract for future schema bumps. When V2 ships, the match arm in `migrate_to_current` grows a `V1 → V2` transform and old-file readers transparently call `migrate_to_current` before `cadk::decode`.
- **Design note** — `migrate_to_current` deliberately bypasses `cadk::inspect()` / `CadkHeader::is_supported()` because those reject unknown schema versions, which is exactly the opposite of what a migrator needs. Instead, the migrator does a minimal cheap parse of magic + the 4-byte `schema_version` field to dispatch to the right transform; only after migration does the payload feed into the regular validation path.
- **`pub use cadk::{SchemaVersion, migrate_to_current}`** added at the cadk-module level so callers do not need to know the submodule path.
- **6 unit tests** in `crates/api/src/cadk/migrate.rs`: schema-version round-trip through `u32`, `SchemaVersion::current()` equals `header::SCHEMA_VERSION`, `from_u32` rejects unknown values (0, 2, `u32::MAX`), and the migrator surfaces `ApiError::Codec` on truncation / bad magic / unknown schema version.
- **`crates/api/tests/cadk_migrate.rs`** — 4 integration tests. Freshly-encoded V1 bytes round-trip byte-identically, compression + thumbnail flags survive the round-trip (flags still set after re-inspect), the committed v0 golden fixture round-trips unchanged (locks the no-op invariant against the on-disk pin), and `migrate → inspect → decode` recovers the original command log.
- STOP_LIST clean (no new `Command` / `Outcome` variants, no new format crate).
...
- **Structured stdout** — `schema:` / `flags: 0xNN` / per-bit flag breakdown / `blobs: N` / `document: N bytes (zstd)` / `thumbnail: present, N bytes (per manifest)` driven by `CadkSummary` fields. Verbose mode continues to dump the full command listing as before.
- **`crates/api/tests/cadk_inspect_bin.rs`** — 7 spawn-the-binary smoke tests via `env!("CARGO_BIN_EXE_cadk-inspect")`: default run on the v0 fixture exits 0 with `(all CRCs verified)`, `--quick` skips decode and downgrades the status line, `-v` prints `--- commands ---` + the `CreateBox` token, missing-path yields exit 1, garbage-on-disk yields exit 2, unknown flag yields exit 1, and `--quick -v` emits the conflict note on stderr while still succeeding.
- STOP_LIST clean (no new `Command` / `Outcome` variants, no new format crate). No library API change — the new surface is entirely on the existing binary.
...
- **`crates/api/tests/cadk_inspect_api.rs`** — 9 integration tests: uncompressed v0 layout reports the right schema/flags/blob counts, compressed container surfaces `DOCUMENT_COMPRESSED` and the post-zstd `document_length`, thumbnail combo reports `blob_count == 2` and the embedded length, compressed-with-thumbnail composes both flags, and the failure modes (bad magic, truncation, manifest CRC corruption, unsupported must-understand flag) all surface as `ApiError::Codec`. One test pins the cheapness invariant: `inspect()` succeeds even when the document body is corrupted, while `decode()` rejects the same bytes.
- STOP_LIST clean (no new `Command` / `Outcome` variants, no new format crate). v0 golden fixture continues to pass under the unchanged `cadk_v0_migration.rs` guard.
...
- **Schema**: `command_schemas()` exposes `op = "solid_label"` with a single required `id: solid_id` parameter so AI/MCP clients can discover the surface.
- **Tests**: 5 new regression tests in `crates/api/tests/api_integration.rs` (default box label is non-empty, `Rename` round-trip reflected through `SolidLabel`, unknown id → `UnknownSolid`, history-untouched, JSON round-trip with `"op":"solid_label"`) + schema-coverage fixture extended.
