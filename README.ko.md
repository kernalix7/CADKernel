...
- **네이티브 형식** — 자체 `.cadkernel` 형식으로 모든 기능과 메타데이터를 무손실 저장
...
| **GUI** | egui / iced *(미정)* | 크로스 플랫폼 immediate / retained 모드 GUI |
...
| **Service** | 모델링 오퍼레이션, 렌더링, 파일 I/O | `cadkernel-modeling`, `cadkernel-render`, `cadkernel-io` |
| **Extension** | 플러그인 로드/관리, API 노출 | `cadkernel-extension` |
| **Application** | GUI, CLI, 스크립팅, AI 연동 | `cadkernel-app`, `cadkernel-mcp` |
| **Platform** | OS 및 하드웨어 추상화 | `cadkernel-platform` |
...
| 형식 | 확장자 | 읽기 | 쓰기 | 비고 |
|------|--------|:----:|:----:|------|
| CADKernel | `.cadkernel` | 🔲 | 🔲 | 무손실 네이티브 형식 |
...
- [ ] 프로젝트 구조 및 빌드 시스템 구성 (Cargo workspace)
- [ ] 코어 수학 라이브러리 (벡터, 행렬, 변환)
- [ ] 기본 기하 엔진 (B-Rep, NURBS 커브/서피스)
- [ ] 기본 위상 구조 (Vertex, Edge, Face, Shell, Solid)
- [ ] 기본 렌더링 파이프라인 (wgpu)
- [ ] 네이티브 형식 (`.cadkernel`) 읽기/쓰기
- [ ] STL 임포트/익스포트 (ASCII / Binary)
- [ ] OBJ 임포트/익스포트
- [ ] CLI 인터페이스

### Phase 2 — 핵심 기능 (Core Features)
- [ ] 3D 솔리드 모델링 오퍼레이션 (Extrude, Revolve, Sweep, Loft)
- [ ] 불리언 연산 (Union, Subtract, Intersect)
- [ ] 제약 조건 솔버 (Constraint Solver)
- [ ] Undo/Redo 시스템
- [ ] GUI 프레임워크 기본 구현
- [ ] STEP AP203/AP214 임포트/익스포트
- [ ] IGES 임포트/익스포트
- [ ] glTF / GLB 임포트/익스포트
- [ ] 3MF 임포트/익스포트
- [ ] SVG / PDF 2D 도면 내보내기

### Phase 3 — 호환성 강화: 산업 표준 (Industry Standard)
- [ ] STEP AP242 (PMI/GD&T) 지원
- [ ] DXF/DWG 임포트/익스포트
- [ ] 3DM (OpenNURBS / Rhino) 임포트/익스포트
- [ ] Parasolid (`.x_t`, `.x_b`) 임포트/익스포트
- [ ] ACIS SAT/SAB 임포트/익스포트
- [ ] JT (Siemens) 임포트/익스포트
- [ ] IFC (BIM / 건축) 임포트/익스포트
- [ ] BREP (OpenCASCADE) 임포트/익스포트
- [ ] FCStd (FreeCAD) 임포트/익스포트
- [ ] 크로스 플랫폼 테스트 및 안정화 (Windows / macOS / Linux)

### Phase 4 — 호환성 강화: 상용 CAD 및 확장 형식 (Commercial CAD & Extended Formats)
- [ ] SolidWorks (`.sldprt`, `.sldasm`) 임포트
- [ ] CATIA V5 (`.catpart`, `.catproduct`) 임포트
- [ ] PTC Creo (`.prt`, `.asm`) 임포트
- [ ] Autodesk Inventor (`.ipt`, `.iam`) 임포트
- [ ] Autodesk Fusion 360 (`.f3d`) 임포트
- [ ] SketchUp (`.skp`) 임포트/익스포트
- [ ] Bentley MicroStation (`.dgn`) 임포트/익스포트
- [ ] Blender (`.blend`) 임포트
- [ ] FBX 임포트/익스포트
- [ ] COLLADA (`.dae`) 임포트/익스포트
- [ ] 3DS 임포트/익스포트
- [ ] VRML / X3D 임포트/익스포트
- [ ] USD / USDA / USDC 임포트/익스포트
- [ ] PLY / OFF 메시 임포트/익스포트
- [ ] AMF 임포트/익스포트
- [ ] EPS / HPGL 2D 내보내기
- [ ] G-code / SLC 제조 출력

### Phase 5 — 포인트 클라우드, 스캔 데이터 및 이미징 (Point Cloud, Scan Data & Imaging)
- [ ] PCD (Point Cloud Library) 임포트/익스포트
- [ ] LAS / LAZ (LiDAR) 임포트/익스포트
- [ ] E57 (3D 스캔) 임포트/익스포트
- [ ] XYZ / PTS (ASCII 포인트 클라우드) 임포트/익스포트
- [ ] PNG / JPEG / BMP / TIFF 렌더 내보내기
- [ ] HDR / EXR 환경 맵 지원

### Phase 6 — 확장 생태계 (Extension Ecosystem)
- [ ] Plugin API 설계 및 구현
- [ ] Add-on 매니저 및 레지스트리
- [ ] Lua/Python 스크립팅 인터페이스
- [ ] 커뮤니티 Add-on 마켓플레이스
- [ ] Add-on API를 통한 커스텀 파일 형식 임포트/익스포트

### Phase 7 — AI 통합 (AI Integration)
- [ ] MCP 서버 구현
- [ ] 자연어 → 모델링 명령 변환
- [ ] AI 기반 설계 어시스턴트
- [ ] 자동화된 설계 검증
...
- 데모 빌드와 스크린샷은 현재 준비 중입니다.
- 첫 공개 알파 전까지 이 섹션에 프리뷰 이미지와 사용 영상을 순차적으로 추가할 예정입니다.
