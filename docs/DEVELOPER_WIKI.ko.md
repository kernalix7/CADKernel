...
### 심화 품질 개선
불리언: `boolean_op`에 자동 면 분할 (split→classify→evaluate), 다중 샘플 분류 (7점 다수결). 스케치: 야코비안 랭크 DOF 분석, `drag_solve()` 대화형 점 드래그. 뷰어: Moller-Trumbore 레이 피킹 (`picking.rs`), undo/redo 명령 스택 (`command.rs`, `ModelSnapshot` 기반).
