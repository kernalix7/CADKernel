...
**UI-A4 — Verify-first Surface/FEM/TechDraw (2026-05-14): 3,351 → 3,356 / 0 / 1.** Fourth consecutive verify-first lane. All 27 dispatcher arms across §3.4 Surface (1: `S::Coons`), §3.5 FEM (2: `FemAction::Summary`/`Report`), §3.6 TechDraw (24) confirmed already wired. Cumulative verify-first counter: 64/64. Additionally, 24 of 27 arms had non-trivial prior test coverage; only 3 needed strengthening. New test files: `crates/viewer/tests/fem_easy_features.rs` (4 tests asserting FEM Summary/Report status-text contract) and `crates/viewer/tests/techdraw_done_features.rs` (1 test asserting `T::Redraw` no-sheet warning-text). Test-support accessor `CadApp::status_message() -> &str` (`#[doc(hidden)]`) added to `crates/viewer/src/app.rs` (+5 lines). No Command/Outcome/GuiAction variants added; no kernel changes. STOP_LIST clean.
